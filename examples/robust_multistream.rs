...
fn main() -> DslResult<()> {
        ...
        for entry in fs::read_dir(&source_path).map_err(|e| {
            ...
            if path.is_file() {
                if let Some(ext) = path.extension() {
                    ...
                    if matches!(ext_str.as_str(), "mp4" | "mkv" | "avi" | "mov" | "webm" | "flv" | "ts" | "m4v") {
    ...
    info!("All pipelines setup complete. Processing {} streams...", stream_ids.len());
    ...
    loop {
        ...
        for (index, stream_id) in stream_ids.iter().enumerate() {
            if let Some(health) = stream_manager.get_stream_health(stream_id) {
                info!("  Stream {}: State={:?}, Errors={}, Recovery Attempts={}", 
                      index, 
                      health.state, 
                ...
                if health.metrics.fps > 0.0 || health.metrics.bitrate > 0 {
                    info!("    Metrics: FPS={:.1}, Bitrate={:.1} kbps", 
                          health.metrics.fps, 
        ...
        if iteration >= 30 {  // Run for about 60 seconds
