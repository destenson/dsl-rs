...
impl FileSinkRobust {
    ...
    async fn rotate_file(&mut self) -> DslResult<()> {
        ...
        // Set new location
        self.filesink.set_property("location", new_file.to_str().unwrap());
        ...
        if let Ok(entries) = fs::read_dir(&self.config.directory) {
            for entry in entries.filter_map(Result::ok) {
                ...
                if let Some(filename) = path.file_name() {
                    ...
                    if filename_str.starts_with(&format!("{}_{}", 
                        self.config.base_filename, self.name)) 
    ...
    async fn check_rotation_needed(&self) -> bool {
        ...
        if self.config.enable_size_rotation {
            ...
            if current_size >= self.config.max_file_size {
                debug!("File size {} exceeds max {}, rotating", 
        ...
        if self.config.enable_time_rotation {
            ...
            if elapsed >= self.config.rotation_interval {
                debug!("Time elapsed {:?} exceeds interval {:?}, rotating", 
...
impl Sink for FileSinkRobust {
    ...
    async fn prepare(&mut self) -> DslResult<()> {
        *self.state.lock().unwrap() = StreamState::Starting;
        ...
        self.filesink.set_property("location", filename.to_str().unwrap());
        ...
        *self.state.lock().unwrap() = StreamState::Running;
        info!("File sink {} prepared, writing to {:?}", self.name, filename);
    ...
    fn metrics(&self) -> StreamMetrics {
        ...
        metrics.bitrate = (*self.bytes_written.lock().unwrap() * 8) / 
            (metrics.uptime.as_secs() + 1); // Avoid division by zero
        metrics
    }
