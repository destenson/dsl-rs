...
use crate::core::{
    DslError, DslResult, Sink, StreamState, StreamMetrics, RecoveryAction
};
...
impl RtspSinkRobust {
    pub fn new(name: String, config: RtspServerConfig) -> DslResult<Self> {
        ...
        let rtsp_sink = gst::ElementFactory::make("rtspclientsink")
            ...
            .property("location", format!("rtsp://127.0.0.1:{}{}", 
    ...
    async fn setup_server(&mut self) -> DslResult<()> {
        ...
        let mounts = server.mount_points().ok_or_else(|| 
            DslError::Sink("Failed to get mount points".to_string()))?;
        ...
        info!("RTSP server started on port {} at {}", 
    ...
    fn build_launch_string(&self) -> String {
        ...
        launch.push_str(&format!("key-int-max={} ! ", self.config.key_frame_interval * 30));
    ...
    fn setup_client_signals(&self, factory: &gst_rtsp_server::RTSPMediaFactory) {
        ...
        factory.connect_media_configure(move |_factory, media| {
            ...
            media.connect_new_stream(move |_media, stream| {
                ...
                clients.lock().unwrap().insert(client_id.clone(), client_info);
                *total.lock().unwrap() += 1;
    ...
    async fn handle_client_disconnect(&self, client_id: &str) {
        if let Some(client) = self.clients.lock().unwrap().remove(client_id) {
            ...
            info!("Client {} disconnected from {} after {:?}", 
...
impl Sink for RtspSinkRobust {
    ...
    async fn cleanup(&mut self) -> DslResult<()> {
        *self.state.lock().unwrap() = StreamState::Stopped;
        ...
        let client_ids: Vec<String> = self.clients.lock().unwrap()
            .keys().cloned().collect();
        ...
        // Stop sink element
        self.sink_element.set_state(gst::State::Null)
...
mod tests {
    ...
    fn test_client_tracking() {
        ...
        sink.clients.lock().unwrap().insert("test_client".to_string(), client_info);
